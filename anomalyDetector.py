...
def fit_norm_distribution_param(args, model, train_dataset, channel_idx=0):
    ...
    with torch.no_grad():
        ...
        model.eval()        
        chain_number = train_dataset.size(0)
        
        if chain_number > 20:
            chain_number = 20
        
        for n in range(chain_number):
        
            pasthidden = model.init_hidden(1)
            
            chain_length = train_dataset.size(1)
            for t in range(chain_length):
                out, hidden = model.forward(train_dataset[n][t].unsqueeze(0).unsqueeze(0).to(device).float(), pasthidden)
                predictions.append([])
                organized.append([])
                errors.append([])
                ...
                for prediction_step in range(1,args.prediction_window_size):
                    ...
                    predictions[n*chain_length + t].append(out.data.cpu()[0][0][channel_idx])
        
                if t >= args.prediction_window_size:
                    for step in range(args.prediction_window_size):
                        organized[n*chain_length + t].append(predictions[step+t-args.prediction_window_size][args.prediction_window_size-1-step])
                    organized[n*chain_length + t]= torch.FloatTensor(organized[n*chain_length + t]).to(args.device)
                    errors[n*chain_length + t] = organized[n*chain_length + t] - train_dataset[n][t][channel_idx]
                    errors[n*chain_length + t] = errors[n*chain_length + t].unsqueeze(0)
                    
            print("\nthe errors of n chains has been calculated", n)
